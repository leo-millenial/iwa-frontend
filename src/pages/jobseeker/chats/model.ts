...
export const currentRoute = routes.jobseeker.chats;

export const statusChanged = createEvent<ChatStatus>();
export const chatOpenClicked = createEvent<{ jobseekerId: string; chatId: string }>();
...
sample({
  ...
  fn: ({ jobseekerId, chatId }) => ({ params: { jobseekerId, chatId } }),
  target: routes.jobseeker.chat.open,
});
